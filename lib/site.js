...
var SiteHandlebars   = require('./broccoli-taco-handlebars');
...
Site.prototype.loadHelpers = function () {
  ...
  var helpers = requireDir(helpersPath);
  var key;
  for (key in helpers) ret[key] = helpers[key];

  //throw new Error('ret: ' + Object.keys(ret));
  return ret;
};
...
Site.prototype.handlebars = function () {
  ...
  return new SiteHandlebars([this.handlebarsTree], handlebarsPagesGlobs, options);
};
...
Site.prototype.toTree = function () {
  ...
  if (assets) {
    ...
    //this.manifest = null; //HACK so the smoke tests execute 
